...
function getConsultationDate(c: Consultation): Date | null {
  ...
  if (c?.scheduledAt && typeof c.scheduledAt === 'object') {
    raw = (c.scheduledAt as any).$date ?? (c.scheduledAt as any).date ?? undefined;
...
function getLastNConsecutiveMonthsData(n: number, monthData: Array<{ name: string, value: number }>) {
  const now = new Date();
  const months = [];
  for (let i = n - 1; i >= 0; i--) {
...
export default function AdminReportsPage() {
    ...
    async function fetchConsultations() {
      ...
      try {
        ...
        if (Array.isArray(cons)) {
          ...
          setConsultations([]);
          setError('No consultation data found in API response. API shape: ' + JSON.stringify(cons));
        }
      } catch (e: any) {
  ...
  if (error) return <div style={{ color: 'red', fontWeight: 700 }}>{error}</div>;
  ...
  const totalConsultations = statusData.reduce((acc, cur) => acc + cur.value, 0);
  const statusPercents = statusData.map(d => ({
    ...d,
    percent: totalConsultations ? Math.round((d.value / totalConsultations) * 100) : 0
  }));
  ...
  consultations.forEach(c => {
    let name =
      typeof c?.instructor === 'object' && c.instructor?.name
        ? c.instructor.name
        : (typeof c?.instructor === 'string' ? c.instructor : 'Unknown');
    instructorCounts[name] = (instructorCounts[name] || 0) + 1;
  });
  const instructorData = Object.entries(instructorCounts).map(([name, value]) => ({ name, value }));
  ...
  let minYear = Infinity, minMonth = 12, maxYear = -Infinity, maxMonth = 1;
  consultations.forEach(c => {
  ...
  const rows = [
    latestSixCards.slice(0, 3),
    latestSixCards.slice(3, 6)
  ];
  ...
  const avgPerInstructor = instructorData.length ? Math.round(totalConsultations / instructorData.length) : 0;
      ...
      <div style={{
        ...
        <section style={{
          ...
          {statusPercents.length ? (
            ...
            <div style={{ color: '#444', padding: '1rem', textAlign: 'center', fontStyle: 'italic' }}>
              ...
              >
                ...
                <Bar dataKey='value'>
                  {instructorData.map((_, idx) => (
                    <Cell key={`cell-bar-${idx}`} fill={BAR_COLORS[idx % BAR_COLORS.length]} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
        ...
        <section style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div>Total Consultations: <strong>{totalConsultations}</strong></div>
              <div>Total Instructors: <strong>{instructorSet.size}</strong></div>
              <div>Total Students: <strong>{studentSet.size}</strong></div>
              <div>Average Consultations: <strong>{avgPerInstructor}</strong></div>

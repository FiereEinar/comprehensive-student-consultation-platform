...
function mapStatusLabel(raw: string): string {
  ...
  if (s.includes('complet') || s === 'done' || s.includes('finish')) return 'Completed';
...
export default function AdminReportsPage() {
  ...
  useEffect(() => {
    async function loadAll() {
      ...
      try {
        ...
        if (!consRes.ok) {
          throw new Error(`Consultations API failed: ${consRes.status} ${consRes.statusText}`);
        }
        ...
        console.log('Sample statuses from DB:', cons.slice(0, 10).map(c => c.status));

        // ---- Users (for instructor names)
        const userRes = await fetch('/api/v1/user');
        if (!userRes.ok) {
          throw new Error(`Users API failed: ${userRes.status} ${userRes.statusText}`);
        }
  ...
  consultations.forEach(c => {
    const date = getDateFromShape(c.scheduledAt) || getDateFromShape(c.createdAt);
    if (!date) return;
      ...
      <div style={{
        ...
        <section style={{
          ...
          {statusPercents.length ? (
            <div style={{ width: '100%', height: '100%', minHeight: chartHeight }}>
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Tooltip formatter={(v: any, name: any, props: any) => [`${props.payload.percent}% (${v})`, name]} />
                </PieChart>
              </ResponsiveContainer>
            </div>
          ) : (
